...
- [课程概述](#课程概述)
- [一、核心基础概念（前置知识）](#一核心基础概念前置知识)
  - [1.1 导数与偏导数](#11-导数与偏导数)
  - [1.2 数值导数 vs 解析导数](#12-数值导数-vs-解析导数)
  - [1.3 梯度与反向传播](#13-梯度与反向传播)
  - [1.4 tanh 激活函数（与 tan 的区别）](#14-tanh-激活函数与-tan-的区别)
- [二、核心代码实现（完整可运行）](#二核心代码实现完整可运行)
  - [2.1 Value 类（自动微分核心）](#21-value-类自动微分核心)
  - [2.2 单个神经元实现](#22-单个神经元实现)
  - [2.3 多层感知机（MLP）实现](#23-多层感知机mlp实现)
  - [2.4 神经网络训练（梯度下降）](#24-神经网络训练梯度下降)
- [三、关键疑问解答（高频问题）](#三关键疑问解答高频问题)
  - [3.1 _backward 前面的下划线是什么意思？](#31-_backward-前面的下划线是什么意思)
  - [3.2 为什么梯度计算用 += 而不是 = ？](#32-为什么梯度计算用--而不是-)
  - [3.3 拓扑排序的作用是什么？](#33-拓扑排序的作用是什么)
- [四、课程核心总结](#四课程核心总结)
  - [4.1 核心逻辑链条](#41-核心逻辑链条)
  - [4.2 关键收获](#42-关键收获)
  - [4.3 延伸思考](#43-延伸思考)
- [五、补充说明](#五补充说明)
...
- **梯度**：将多变量函数的所有偏导数打包成向量（如 $\nabla f = \left(\frac{\partial f}{\partial a}, \frac{\partial f}{\partial b}, \frac{\partial f}{\partial c}\right)$），描述输入变量对输出的整体影响。
...
- **tan（正切）**：三角函数，公式 $\tan(x) = \frac{\sin x}{\cos x}$，值域 $(-\infty,+\infty)$，周期震荡、有断点，无法用于神经网络激活。
